define([], function () {
  ...
  function injectEVRStylesheet(definitions) {
    ...
    if (definitions.evrBackgroundColorByLevel) {
      Object.keys(definitions.evrBackgroundColorByLevel).forEach(function (level) {
        var levelColor = definitions.evrBackgroundColorByLevel[level];

        var style = BG_LEVEL_TEMPLATE.replace('$LEVEL', level.toLowerCase()).replace(
          '$COLOR',
          levelColor
        );
    ...
    if (definitions.evrForegroundColorByLevel) {
      Object.keys(definitions.evrForegroundColorByLevel).forEach(function (level) {
        var levelColor = definitions.evrForegroundColorByLevel[level];

        var style = FG_LEVEL_TEMPLATE.replace('$LEVEL', level.toLowerCase()).replace(
          '$COLOR',
          levelColor
        );
