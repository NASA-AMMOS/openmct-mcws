...
class MessageStreamProcessor {
    ...
    processSuspectChannels(message) {
        ...
        Object.keys(this.markedSuspectChannels).forEach(markedSuspectChannel => {
            if(!currentSuspectChannelsObject[markedSuspectChannel]) {
                ...
                this.notificationService.alert(`Following Channel is not marked as suspect anymore - ${markedSuspectChannel}`);
            }
        });
    }
    ...
    clearData(message) {
        ...
        this.notificationService.info(`Data Cleared On All Displays By ${message.message_type} Message`);
    }
