...
define(['./LMSTFormat'], function (LMSTFormat) {
  ...
  describe('LMSTFormat', function () {
    var SOL0, millisecondsPerSol, format;

    beforeEach(function () {
      SOL0 = Date.UTC(2020, 2, 18, 0, 0, 0);
      millisecondsPerSol = 88775244;
      format = new LMSTFormat(SOL0);
    });
    ...
    describe('parse', function () {
      ...
      it('converts length of days appropriately', function () {
        expect(format.parse('SOL-0001M00:00:00.000')).toEqual(SOL0 + millisecondsPerSol);
      });

      it('handles partial inputs', function () {
        expect(format.parse('SOL-0')).toEqual(SOL0);
        expect(format.parse('SOL-0001')).toEqual(SOL0 + millisecondsPerSol);
      });
    });
    ...
    describe('format', function () {
      ...
      it('can handle days correctly', function () {
        expect(format.format(SOL0 + millisecondsPerSol)).toEqual('SOL-0001M00:00:00.000');
      });
    });
    ...
    it('zero pads sol', function () {
      expect(format.format(format.parse('SOL-12M12:00:00.000'))).toBe('SOL-0012M12:00:00.000');
    });

    describe('odd behaviors', function () {
      it('is not off by 1 ms', function () {
        expect(format.format(format.parse('SOL-0318M15:55:05.646'))).toBe('SOL-0318M15:55:05.646');

        expect(format.format(format.parse('SOL-0217M15:55:05.646'))).toBe('SOL-0217M15:55:05.646');

        expect(format.format(format.parse('SOL-0117M15:55:05.646'))).toBe('SOL-0117M15:55:05.646');

        expect(format.format(format.parse('SOL-0017M15:55:05.646'))).toBe('SOL-0017M15:55:05.646');
      });
    });
  });
});
