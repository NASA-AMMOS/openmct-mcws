...
describe('Historical Provider', function () {
  ...
  beforeEach(function () {
    ...
    mockOpenMct.telemetry = jasmine.createSpyObj('telemetryApi', ['getMetadata']);
    mockMetadata = jasmine.createSpyObj('metadata', ['value', 'values']);
    ...
    mockSessionService = jasmine.createSpyObj('SessionService', ['getHistoricalSessionFilter']);
    spyOn(HistoricalProvider.prototype, 'getSessionService').and.returnValue(mockSessionService);

    mockOpenMct.notifications = jasmine.createSpyObj('notificationAPI', ['alert']);
    mockNotification = jasmine.createSpyObj('notification', ['on', 'off']);
