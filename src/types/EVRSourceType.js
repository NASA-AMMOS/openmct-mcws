define(['./VISTAType', '../constants', 'lodash'], function (VISTAType, constants, _) {
  /**
  ...
  var EVRSourceType = new VISTAType({
    ...
    getComposition: function (domainObject, dataset, data, types) {
      if (dataset.hasEVRs()) {
        return Promise.all([dataset.evrs.getModules(), dataset.evrs.getLevels()]).then(
          function (results) {
            return results[1].concat(results[0]).map(function (module) {
    ...
    makeObject: function (dataset, data) {
      if (dataset.hasEVRs()) {
        return dataset.evrs.getModules().then(
          function (modules) {
            var object = {
              type: this.key,
            ...
            _.assignIn(object.telemetry, _.pick(dataset.options, constants.EVR_PROPERTIES));
