...
function makeEnumerationRange(enumerationList) {
  const enumerations = enumerationList.map((enumDefinition) => {
    return {
      string: enumDefinition.enum_string,
      value: Number(enumDefinition.enum_value)
    };
  });
  const enumerationRange = {
    key: 'enum',
    name: 'Enum',
    type: 'enum',
    format: 'enum',
...
function buildChannelObject(channel, object) {
  ...
  if (channel.enumerations && channel.enumerations.length) {
    object.telemetry.values.push(makeEnumerationRange(channel.enumerations));
    object.telemetry.values.push({
      key: 'status',
      name: 'Status',
      type: 'string',
  ...
  } else if (channel.data_type === 'BOOLEAN' || channel.data_type === 'BOOL') {
    object.telemetry.values.push({
      key: 'enum',
      name: 'Enum',
      type: 'enum',
 ...
 */
const ChannelType = new VISTAType({
  key: 'vista.channel',
  name: 'Channel',
  ...
  getLocation: function (dataset, data) {
    return ChannelGroupType.makeIdentifier(data.channelId.split('-')[0], data.datasetIdentifier);
  },
  makeObject: function (dataset, data) {
    ...
    if (dataset.hasChannels()) {
      Object.assign(object.telemetry, dataset.channels.urls);
      return dataset.channels.getChannel(data.channelId).then((channel) => {
