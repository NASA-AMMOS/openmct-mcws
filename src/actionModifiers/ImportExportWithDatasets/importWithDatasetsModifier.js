...
function importWithDatasetsModifier(openmct) {
  openmct.forms.addNewFormControl(
    'import-with-datasets-controller',
    getImportWithDatasetsFormController(openmct)
  );
  ...
  function getReferencedDatasetsFromImport(json) {
    ...
    Object.values(objects).forEach((object) =>
      object.composition?.forEach((identifier) => {
        if (Types.hasTypeForIdentifier(identifier)) {
          ...
          const isAdded = referencedDatasetsFromImport.some((dataset) =>
            openmct.objects.areIdsEqual(dataset.identifier, datasetIdentifier)
          );
