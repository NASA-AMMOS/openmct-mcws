...
export default async function oldPersistenceFolderInterceptor(openmct, namespaces, ROOT_IDENTIFIERS) {
    ...
    const userKeyRegex = usersNamespace ? new RegExp(`^${usersNamespace.childTemplate.key.split('$')[0]}([^:]+)$`) : null;
    ...
    openmct.objects.addGetInterceptor({
        appliesTo: (identifier, domainObject) => {
            ...
            const isRootFolder = nonUserRoots.some(rootId => openmct.objects.areIdsEqual(rootId, identifier));
        ...
        invoke: (identifier, object) => {
            ...
            if (isUserNamespace(usersNamespace, userKeyRegex, identifier) && !identifier.namespace.includes('shared')) {
