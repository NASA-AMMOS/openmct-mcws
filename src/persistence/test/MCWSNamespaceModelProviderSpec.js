...
describe('MCWS Providers', () => {
  ...
  beforeEach(() => {
    ...
    namespaces = [someNamespace, anotherNamespace, personalContainerNamespace, personalNamespace];
    ...
    spyOn(userContainerProvider, 'getPersistenceNamespaces').and.returnValue(
      Promise.resolve(namespaces)
    );
    spyOn(userContainerProvider, 'getContainedNamespaces').and.callFake((namespace) => {
  ...
  describe('MCWSPersistenceProvider', () => {
    ...
    beforeEach(() => {
      ...
      spyOn(persistenceProvider, 'getPersistenceNamespaces').and.returnValue(
        Promise.resolve(namespaces)
      );
    ...
    it('handles abort signal when getting objects', async () => {
      ...
      expect(mcws.namespace).toHaveBeenCalledWith(jasmine.any(String), { signal: abortSignal });
    });
    ...
    it('creates new objects', async () => {
      ...
      expect(mcwsNamespace.opaqueFile('new-object').create).toHaveBeenCalledWith(expectedModel);
    });
    ...
    it('updates existing objects', async () => {
      ...
      expect(success).toBe(true);
      expect(mcwsNamespace.opaqueFile('existing-object').replace).toHaveBeenCalledWith(
        expectedModel
      );
    });
