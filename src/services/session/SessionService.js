...
class SessionService {
  ...
  async getTopicsWithSessions() {
    ...
    const topicsWithSessions = await Promise.all(
      sessionLADUrls.map((url) => this.getActiveSessions(url))
    );
  ...
  async getHistoricalSessions(filters, urls) {
    ...
    const sessionRequests = sessionUrls.map((sessionUrl) =>
      this.getHistoricalSessionsByUrl(sessionUrl, filters)
    );
  ...
  notifyUserOfHistoricalSessionFilterChange(model) {
    ...
    if (model) {
      notificationString = `Historical queries restricted to ${model.numbers.length} ${model.numbers.length > 1 ? "session's" : 'session'} (${formatMultipleSessionNumbers(model.numbers)}) on ${model.host}.`;
    }
