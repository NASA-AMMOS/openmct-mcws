...
xdescribe('SessionSelectorController', function () {
  ...
  beforeEach(function () {
    ...
    sessionService.getTopicsWithSessions.and.returnValue(
      Promise.resolve([
        {
          topic: 'AMPCS.a',
          jms_subtopic: 'a',
  ...
  describe('loading with selected topic', function () {
    beforeEach(function (done) {
      $scope.$digest.and.callFake(done);
      sessionService.isActiveTopic.and.callFake(function (model) {
  ...
  describe('loading with selected session', function () {
    beforeEach(function (done) {
      $scope.$digest.and.callFake(done);
      sessionService.isActiveSession.and.callFake(function (model) {
        return model.topic === 'AMPCS.a' && model.number === 2;
      });
      controller = new SessionSelectorController($scope, sessionService);
    });
  ...
  describe('selecting a session', function () {
    ...
    it('changes active topic when confirmed', function () {
      controller.confirm();
      expect(sessionService.setActiveTopicOrSession).toHaveBeenCalledWith($scope.selection.data);
      expect($scope.ngModel.dismiss).toHaveBeenCalled();
    });

    it('does not change active topic when canceled', function () {
      controller.cancel();
      expect(sessionService.setActiveTopicOrSession).not.toHaveBeenCalled();
      expect($scope.ngModel.dismiss).toHaveBeenCalled();
    });
  });
