...
xdescribe('SessionIndicatorController', function () {
  ...
  beforeEach(function () {
    ...
    overlayService = jasmine.createSpyObj('overlayService', ['createOverlay']);
    overlay = jasmine.createSpyObj('overlay', ['dismiss']);
    ...
    controller = new SessionIndicatorController($scope, sessionService, overlayService, $timeout);
  });
  ...
  it('listens for and handles changes of active session', function () {
    ...
    expect(sessionService.listen).toHaveBeenCalledWith(jasmine.any(Function));

    sessionService.listen.calls.mostRecent().args[0]('anotherSession');
    expect($scope.activeSession).toBe('anotherSession');
    expect(controller.pollForSessions.calls.all().length).toBe(callsToPoll + 1);
  });
  ...
  describe('pollForSessions', function () {
    ...
    it('sets a timeout to poll sessions', function () {
      ...
      expect($timeout).toHaveBeenCalledWith(jasmine.any(Function), jasmine.any(Number));
    ...
    it('does not fetch sessions with active session', function () {
      expect($scope.activeSession).toBe('mySession');
      controller.pollForSessions();
      expect(sessionService.getTopicsWithSessions).not.toHaveBeenCalled();
    });
    ...
    describe('without active session', function () {
      ...
      it('sets hasTopic to true when topics exist', function () {
        sessionService.getTopicsWithSessions.and.returnValue(Promise.resolve(['a', 'b']));
        controller.pollForSessions();
        return digestPromise.then(() => {
  ...
  describe('selectSession', function () {
    ...
    it('opens an overlay and provides dismiss function', function () {
      expect(overlayService.createOverlay).toHaveBeenCalledWith('vista.sessionSelectorView', {
  ...
  it('can disconnect the active stream', function () {
    ...
    expect(sessionService.setActiveTopicOrSession).toHaveBeenCalledWith(undefined);
  });
});
