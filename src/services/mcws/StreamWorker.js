(function (self, WebSocket) {
   ...
   * @param {string} property the property to filter onß
   * @param {Object} topic metadata about the topic to listen on
   * @constructor
   * @private
   */
  function MCWSConnection(url, property, topic) {
    this.url = url;
    this.topic = topic;
    this.subscribers = {};
  ...
  MCWSConnection.prototype.query = function () {
    var filter = {
      session: this.topic && this.topic.number,
  ...
  MCWSStreamWorker.prototype.subscribe = function (subscription) {
    ...
    if (!this.connections[url]) {
      this.connections[url] = new MCWSConnection(url, property, this.activeTopic);
    }

    this.connections[url].subscribe(key);
  };
  ...
  MCWSStreamWorker.prototype.unsubscribe = function (subscription) {
    ...
    if (this.connections[url]) {
      this.connections[url].unsubscribe(key);
    }
  };
  ...
  MCWSStreamWorker.prototype.topic = function (topic) {
    ...
    Object.keys(this.connections).forEach(
      function (url) {
        this.connections[url].setTopic(topic);
      }.bind(this)
    );
  };
