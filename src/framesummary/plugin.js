...
export default function FrameWatchViewPlugin(options) {
  return function install(openmct) {
    openmct.types.addType(ENCODING_WATCH_TYPE, {
      ...
      description:
        'Drag and drop a Frame Summary Event node into this view to show summary of frame statistics for each unique observed VC and Encoding Family combination.',
    ...
    openmct.types.addType(FRAME_WATCH_TYPE, {
      ...
      description:
        'Drag and drop a Frame Summary Event node into this view to show summary of frame statistics for each unique observed VC and Encoding Type combination.',
      cssClass: 'icon-tabular-lad',
      creatable: true,
      initialize(domainObject) {
        domainObject.composition = [];
    ...
    openmct.inspectorViews.addProvider(
      new FrameWatchConfigurationViewProvider(
        openmct,
    ...
    openmct.objectViews.get = function (domainObject) {
      return wrappedGet
        ...
        .filter(
          (viewProvider) =>
            !(domainObject.type === 'vista.frameSummary' && viewProvider.key === 'table')
        );
    };
  };
}
