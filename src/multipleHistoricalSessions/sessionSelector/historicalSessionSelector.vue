    ...
    <template v-else>
      <div class="c-overlay__top-bar">
        ...
        <div class="c-overlay__dialog-hint">
          Select one or more sessions to use for historical queries. Note that sessions may only be
          selected within one host at a time.
        </div>
      </div>
      <div class="c-overlay__contents-main c-hss__view-wrapper">
        <!-- Left column -->
        <div class="c-hss__section c-hss__hosts section-host">
          ...
          <ul class="c-hss__section__content">
            ...
            >
              <div class="c-tree__item" :class="{ 'is-navigated-object': host === selectedHost }">
      ...
      <div class="c-overlay__button-bar">
        ...
        <a class="c-button" @click="closeOverlay"> Cancel </a>
      </div>
    </template>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    selectHostAndFilterSessions(host) {
      ...
      this.sessionServiceFilteredByHost = this.availableSessions.filter(
    ...
    setMarkedSessions() {
      if (this.sessionFilter.numbers && this.sessionFilter.host) {
        this.availableSessions.forEach((session) => {
          if (
            this.sessionFilter.numbers.some((sessionNumber) => sessionNumber === session.number) &&
