<template>
  <div class="h-indicator">
    ...
    >
      <span v-if="realtimeSessionDisabled" class="c-indicator__label"
        >Real-time Disabled in Config</span
      >

      <span v-else class="c-indicator__label">
        ...
        <span v-if="hasTopics && !hasActiveSession">
          Real-time Available
          <button @click="openSessionSelector()">Select</button>
        </span>
        <span v-if="!hasTopics && !hasActiveSession"> Real-time Not Available </span>
      </span>
    </div>
