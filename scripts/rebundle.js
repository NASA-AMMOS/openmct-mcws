...
var glob = require('glob'),
  ...
  path = require('path'),
  _ = require('lodash'),
  template = _.template(fs.readFileSync(path.resolve(__dirname, 'rebundle-template.txt'), 'utf8'));
...
function replaceImpls(bundleText) {
  ...
  return bundleText
    ...
    .map(function (line) {
      var m = line.match(rx);
      return m !== null ? line.replace(rx, '"implementation": ' + toIdentifier(m[1])) : line;
    })
    .join('\n');
}
