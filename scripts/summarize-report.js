...
var columns = ['Test ID', 'Relevant reqs.', 'Conducted?', 'Passed?', 'Issues'];
var symbols = {
    ...
    columns
      .map(
        function (column) {
          ...
          return transform ? transform(value, this.metrics, title, filename, procedure) : value;
        }.bind(this)
      )
      .join(' | ') + '\n';
...
var sortedRequirements = _(requirements)
  ...
  .sortBy(function (requirement) {
    return requirement.id.replace(/\d+/, function (n) {
      while (n.length < 4) {
        // look guys i rewrote leftpad!
        n = '0' + n;
      }
      return n;
    });
